...
commandlist = ['homepath','delpath','showpath','quit','newhomepath','delfile','createfile','addcontent','store',
            'runfile','runfunc','funclist','findfunc','content','addpath','findpath','browse','_file_','_filereset_',
            '_lines_','_linesreset_','storelines','_fromlines_','RunFile','clearcontent','clr','clear','help',
               'enablebrowser','disablebrowser','enablenhp','disablenhp','quit','history','clearhistory','searchhistory',
               'msg','prompt','yesno','custom_yesno','exit_yesno','menu','box','error','warn','info','tip','star',
               'function','int','string','float','list','tuple','type']

arg0 = ['homepath','showpath','delpath','enablenhp','disablenhp','enablebrowser','disablebrowser','quit',
        'clear','clr','help','_file_','_filereset_','_lines_','_linesreset_','history','clearhistory',
        'error','yesno','exit_yesno']
...
arg1 = ['newhomepath','delfile','createfile','addcontent','store','runfile','funclist','content',
        ...
        'searchhistory','error','warn','info','tip','custom_yesno','prompt','box','msg','star',
        'int','string','float','list','tuple','function','type']
...
while True:
    ...
    if arglen>4:
        print(f"rf4>>Max command length:4  Got:{arglen}")
        continue
    else:
        pass
    ...
    if arglen==1:
        ...
        if command[0] not in arg0:
            if command[0] in arg1:
                ...
                print("rf4>>Command '{}' takes 3 arguments. Enter 'help' for more info.".format(command[0]))
            continue
        ...
        if command[0]=="help":
            ...
            print("_filereset_                        : Clear data stored in _file_.")
            print("_linesreset_                       : Clear data stored in _lines_.")
        ...
        if com_arg[0]=='_lines_':
            com_arg[0] = contentkey.get('_lines_')

        if command[0]=='type':
            try:
                print(type(var[com_arg[0]]))
            except:
        ...
        if command[0]=='_lines_':
            if com_arg[0].isnumeric()==False:
                print("rf4>>Line number required.")
            elif com_arg[0].isnumeric()==True:
                try:
                    com_arg[0] = int(com_arg[0])
                except:
                    print("rf4>>_lines_ accepts numbers only.")
                else:
                    try:
                        print("Line {0}:{1}".format(com_arg[0],contentkey.get('_lines_')[com_arg[0]-1].strip("\n")))
                    except:
        ...
        if command[0]=='_fromlines_':
            if com_arg[0].isnumeric()==False:
                ...
                try:
                    ...
                    try:
                        contentkey['_file_'] = contentkey.get('_lines_')[com_arg[0]-1].strip("\n")
        ...
        if command[0]=='createfile':
            if os.path.exists(path+"\\"+com_arg[0])==True:
                print("rf4>>File '{}' already exists.".format(com_arg[0]))
            else:
                try:
                    ...
                    print("rf4>>File '{}' has been created.".format(com_arg[0]))
            continue
        ...
        if command[0]=='delfile':
            try:
                ...
                print("rf4>>File '{}' doesn't exist.".format(com_arg[0]))
            else:
                print("rf4>>File '{}' has been deleted.".format(com_arg[0]))
            continue
        ...
        if com_arg[1]=='_file_':
            com_arg[1] = contentkey.get('_file_')

        if com_arg[1]=='_lines_':
            com_arg[1] = contentkey.get('_lines_')
